...
#define FRAME_40ms_48 1920
#define MIC_CHAN_NUM (10)
#define MIC_ARRAY_CHANNELS (16)
#define SPK_CHANNELS (1)
#define SPK_CHAN_NUM (2)

//#define NO_KUOZHANMAI
#define FILE_WRITE 1
#define WAVE_TO_TEST 0
...
struct stream_in {
    ...
    char filename[100];
    FILE *aec_out_file;
    FILE *aec_dt_state_file;
    FILE *temp_file;
    FILE *chfiles[MIC_CHAN_NUM];
    FILE *reffiles[SPK_CHAN_NUM];

    void *hAEC;   // sdssgl add for damo AEC
    short *raw_r_buffer;    // read from snd card
    short *pMicPCM[MIC_CHAN_NUM];    // for aec mic 
    short *Speaker_Mic_Ref[SPK_CHAN_NUM];    // for aec SpeakerRef & MicRef
    unsigned int buffer_bytes;
    unsigned int channel_bytes;

    short s_MicBuff[MIC_ARRAY_CHANNELS][FRAME_20ms_48];
