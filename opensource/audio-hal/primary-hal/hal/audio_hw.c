...
/* sdssgl add */
static int is_first_flag_aec = 1;
static int is_first_flag_dl = 1;
static int g_dt_state = 0;
static int aec_dl_sync_flag = 0;
/* sdssgl add end */
...
/* sdssgl add */
/* use dynamic control flags
 * for pcm file write
 * default is close
 * */
static unsigned long file_write_flags = 0;

//ldx add
void *hDL;
#define AEC_DOWNLINK_SYNC 1

#define FILE_WRITE  1
char array_sumbytes[7680]={0};
#if(FILE_WRITE == 1)
FILE *outfiledl = NULL;;
FILE *outfiledlout = NULL;
#if AEC_DOWNLINK_SYNC
FILE *outfileDTS;
#endif
#endif
...
bool audio_hw_send_qdsp_parameter(int stream_type, float vol, bool active)
{
    ...
    ALOGE("%s: exit with ret %d", __func__, ret);
    return ret;
}
...
{
    ALOGE("%s snd device %d", __func__, snd_device);
                ...
                if (curr_out && PCM_PLAYBACK == uc->type && uc != uc_info) {
                    ...
                    */
                    ...
                    if (ret < 0 || num_devices == 0) {
                        ALOGE("%s: Unable to split uc->out_snd_device: %d",__func__, uc->out_snd_device);
                        split_snd_devices[0] = uc->out_snd_device;
                        num_devices = 1;
                    }
                    for (i = 0; i < num_devices; i++) {
                        usecase_backend_idx = platform_get_backend_index(split_snd_devices[i]);
                        ALOGE("%s:snd_dev %d usecase_backend_idx %d",__func__, split_snd_devices[i],usecase_backend_idx);
                        if((new_backend_idx == HEADPHONE_BACKEND) &&
                               ((usecase_backend_idx == HEADPHONE_44_1_BACKEND) ||
                               (usecase_backend_idx == DSD_NATIVE_BACKEND))) {
                            ALOGE("%s:DSD or native stream detected enabling asrcmode in hardware",
                                  __func__);
                            ...
                            ALOGE("%s:48K stream detected, disabling and enabling it with asrcmode in hardware",
                                  __func__);
...
static int enable_disable_effect(struct audio_device *adev, int effect_type, bool enable)
{
    ...
    ALOGE("%s: effect_type:%d enable:%d", __func__, effect_type, enable);
    ...
    ALOGE("%s: %d %d usecase->id:%d usecase->in_snd_device:%d", __func__, effect_config.module_id,
                       ...
                       struct audio_usecase *usecase)
{
    ...
    if (usecase->type == PCM_CAPTURE) {
        ...
        if (in && is_loopback_input_device(in->device)) {
            ALOGE("%s: set custom mtmx params v1", __func__);
    ...
    ALOGE_IF(strlcpy(mixer_path, use_case_table[usecase->id], sizeof(mixer_path))
            >= sizeof(mixer_path), "%s: truncation on mixer path", __func__);
    // this also appends to mixer_path
    platform_add_backend_name(mixer_path, snd_device, usecase);
    ALOGE("%s: apply mixer and update path: %s", __func__, mixer_path);
                        ...
                        struct audio_usecase *usecase)
{
    ...
    if (usecase->type == PCM_CAPTURE) {
        in = usecase->stream.in;
        if (in && is_loopback_input_device(in->device)) {
            ALOGE("%s: reset custom mtmx params v1", __func__);
                      ...
                      snd_device_t snd_device)
{
    ...
    if ((adev->snd_dev_ref_cnt[snd_device] > 1) &&
            (platform_split_snd_device(adev->platform,
                                       ...
                                       &num_devices,
                                       new_snd_devices) != 0)) {
        ALOGE("%s: snd_device(%d: %s) is already active",
    ...
    if (platform_can_enable_spkr_prot_on_device(snd_device) &&
        ...
        ALOGE("%s: snd_device(%d: %s)", __func__, snd_device, device_name);
        ...
        if (SND_DEVICE_OUT_HEADPHONES == snd_device &&
            ...
            ALOGE("%s: %d: napb: enabling native mode in hardware",
                  __func__, __LINE__);
            audio_route_apply_and_update_path(adev->audio_route,
                                              "true-native-mode");
        ...
        if (((snd_device == SND_DEVICE_IN_HANDSET_6MIC) ||
            ...
            ALOGE("%s: init ec ref loopback", __func__);
    ...
    if (adev->snd_dev_ref_cnt[snd_device] == 0) {
        ...
        if (snd_device == SND_DEVICE_OUT_BT_A2DP)
            ...
            adev->is_channel_status_set = false;
                 ...
                 adev->native_playback_enabled) {
            ALOGE("%s: %d: napb: disabling native mode in hardware",
                  __func__, __LINE__);
            audio_route_reset_and_update_path(adev->audio_route,
                                              "true-native-mode");
            adev->native_playback_enabled = false;
                 ...
                 adev->asrc_mode_enabled) {
            ALOGE("%s: %d: disabling asrc mode in hardware", __func__, __LINE__);
            disable_asrc_mode(adev);
            audio_route_apply_and_update_path(adev->audio_route, "hph-lowpower-mode");
        } else if (((snd_device == SND_DEVICE_IN_HANDSET_6MIC) ||
            (snd_device == SND_DEVICE_IN_HANDSET_QMIC)) &&
            (audio_extn_ffv_get_stream() == adev_get_active_input(adev))) {
            ALOGE("%s: deinit ec ref loopback", __func__);
                                               ...
                                               snd_device_t new_snd_device)
{
    ...
    if ((a1 != a2) && (a1 & a2)) {
        ...
        if (ret < 0) {
            if (ret != -ENOSYS) {
                ALOGE("%s failed to split snd_device %d",
                      __func__,
                      popcount(a1) > 1 ? d1 : d2);
            }
            goto end;
                                              ...
                                              snd_device_t snd_device)
{
    ...
    ALOGE("%s:becf: force routing %d", __func__, force_routing);
    ...
    list_for_each(node, &adev->usecase_list) {
        ...
        ALOGE("%s:becf: (%d) check_usecases curr device: %s, usecase device:%s "
        ...
        if ((usecase->type != PCM_CAPTURE) && (usecase != uc_info) &&
            ...
            if (((uc_derive_snd_device != usecase->out_snd_device) || force_routing) &&
                ...
                (platform_check_backends_match(snd_device, usecase->out_snd_device)))) {
                ALOGE("%s:becf: check_usecases (%s) is active on (%s) - disabling ..",
    ...
    ALOGE("%s:becf: check_usecases num.of Usecases to switch %d", __func__,
        num_uc_to_switch);
        ...
        list_for_each(node, &adev->usecase_list) {
            ...
            if (switch_device[usecase->id]) {
                ...
                if (usecase->type != VOICE_CALL) {
                    ALOGE("%s:becf: enabling usecase (%s) on (%s)", __func__,
                                             ...
                                             snd_device_t snd_device)
{
    ...
    list_for_each(node, &adev->usecase_list) {
        ...
        if (usecase->type != PCM_PLAYBACK &&
            ...
            ALOGE("%s: Usecase (%s) is active on (%s) - disabling ..",
                  __func__, use_case_table[usecase->id],
...
static int read_hdmi_sink_caps(struct stream_out *out)
{
    ...
    if (platform_is_edid_supported_format(out->dev->platform, AUDIO_FORMAT_AC3)) {
        ALOGE(":%s HDMI supports AC3/EAC3 formats", __func__);
    ...
    for (j = 0; j < MAX_SUPPORTED_SAMPLE_RATES; j++) {
        if (platform_is_edid_supported_sample_rate(out->dev->platform, out_hdmi_sample_rates[j])) {
            ALOGE(":%s HDMI supports sample rate:%d", __func__, out_hdmi_sample_rates[j]);
                                         ...
                                         uint32_t max_rates __unused)
{
    ...
    for (i=0; i<count; i++) {
        ALOGE("%s %s %d", __func__, is_playback ? "P" : "C",
                                      ...
                                      uint32_t max_masks)
{
    ...
    for (size_t i = 0; i < num_masks; ++i) {
        ALOGE("%s: %s supported ch %d supported_channel_masks[%zu] %08x num_masks %d", __func__,
                                ...
                                uint32_t max_formats __unused)
{
    ...
    ALOGE("%s: %s supported format %d", __func__,
                                                 ...
                                                 usecase_type_t type)
{
    ...
    list_for_each(node, &adev->usecase_list) {
        ...
        if (usecase->type == type) {
            ALOGE("%s: usecase id %d", __func__, usecase->id);
            return usecase->id;
        }
    }
...
bool audio_is_true_native_stream_active(struct audio_device *adev)
{
    ...
    if (NATIVE_AUDIO_MODE_TRUE_44_1 != platform_get_native_support()) {
        ALOGE("%s:napb: not in true mode or non hdphones device",
               __func__);
        active = false;
        goto exit;
    }
    ...
    list_for_each(node, &adev->usecase_list) {
        ...
        if (curr_out && PCM_PLAYBACK == uc->type) {
            ...
            if (is_offload_usecase(uc->id) &&
                ...
                active = true;
                ALOGE("%s:napb:native stream detected", __func__);
            }
        }
    }
exit:
...
static uint32_t adev_init_dsp_bit_width_enforce_mode(struct mixer *mixer)
{
    ...
    if (property_get("persist.vendor.audio_hal.dsp_bit_width_enforce_mode",
       ...
       default:
            ...
            ALOGE("%s Dynamic DSP bitwidth config is disabled.", __func__);
            break;
        }
    }
                                                ...
                                                bool enable)
{
    ...
    if (enforce_mode == 0) {
        ALOGE("%s: DSP bitwidth feature is disabled.", __func__);
        return;
    }
    ...
    ALOGE("%s DSP bit width feature status is %d width=%d",
...
bool audio_is_dsd_native_stream_active(struct audio_device *adev)
{
    ...
    list_for_each(node, &adev->usecase_list) {
        ...
        if (curr_out && PCM_PLAYBACK == uc->type &&
            ...
            active = true;
            ALOGE("%s:DSD playback is active", __func__);
            break;
        }
    }
    return active;
...
static bool force_device_switch(struct audio_usecase *usecase)
{
    ...
    if (is_offload_usecase(usecase->id) &&
        ...
        is_it_true_mode = (NATIVE_AUDIO_MODE_TRUE_44_1 == platform_get_native_support()? true : false);
         if ((is_it_true_mode && !adev->native_playback_enabled) ||
            ...
            ret = true;
            ALOGE("napb: time to toggle native mode");
        }
    }
    ...
    if((usecase->stream.out->devices & AUDIO_DEVICE_OUT_ALL_A2DP) &&
        audio_extn_a2dp_is_force_device_switch()) {
         ALOGE("Force a2dp device switch to update new encoder config");
         ret = true;
    }

    if (usecase->stream.out->stream_config_changed) {
         ALOGE("Force stream_config_changed to update iec61937 transmission config");
         return true;
    }
    return ret;
}
...
bool is_bt_soc_on(struct audio_device *adev)
{
    ...
    ALOGE("BT SOC status: %d",bt_soc_status);
...
int select_devices(struct audio_device *adev, audio_usecase_t uc_id)
{
    ...
    ALOGE("%s for use case (%s)", __func__, use_case_table[uc_id]);
    ...
    if ((is_btsco_device(out_snd_device,in_snd_device) && !adev->bt_sco_on) ||
         (is_a2dp_device(out_snd_device) && !audio_extn_a2dp_source_is_ready())) {
          ALOGE("SCO/A2DP is selected but they are not connected/ready hence dont route");
          return 0;
    }

    if (out_snd_device != SND_DEVICE_NONE &&
            out_snd_device != adev->last_logged_snd_device[uc_id][0]) {
        ALOGE("%s: changing use case %s output device from(%d: %s, acdb %d) to (%d: %s, acdb %d)",
              __func__,
    ...
    if (in_snd_device != SND_DEVICE_NONE &&
        ...
        ALOGE("%s: changing use case %s input device from(%d: %s, acdb %d) to (%d: %s, acdb %d)",
              __func__,
              use_case_table[uc_id],
    ...
    if ((out_snd_device == SND_DEVICE_OUT_SPEAKER_AND_BT_A2DP ||
        ...
        ALOGE("%s: A2DP profile is not ready, routing to speaker only", __func__);
         ...
         if (usecase->type == VOIP_CALL) {
             if (in != NULL && !in->standby) {
                 if (is_bt_soc_on(adev) == false){
                      ALOGE("BT SCO MIC disconnected while in connection");
                      if (in->pcm != NULL)
                          pcm_stop(in->pcm);
                 }
             }
             if ((usecase->stream.out != NULL) && (usecase->stream.out != adev->primary_output)
                  ...
                  if (is_bt_soc_on(adev) == false) {
                      ALOGE("BT SCO/A2DP disconnected while in connection");
...
static int stop_input_stream(struct stream_in *in)
{
    ...
    ALOGE("%s: enter: usecase(%d: %s)", __func__,
    ...
    if(in->aec_dt_state_file)
        fclose(in->aec_dt_state_file);
    if(in->aec_out_file)
        fclose(in->aec_out_file);
    //fclose(in->temp_file);

    in->raw_r_buffer = NULL;
    free(in->raw_r_buffer);
    
    for(int ch = 0; ch < MIC_CHAN_NUM; ch++)
    {
        if(in->chfiles[ch])
            fclose(in->chfiles[ch]);
    }

    for(int ch = 0; ch < SPK_CHAN_NUM; ch++)
    {
        if(in->reffiles[ch])
            fclose(in->reffiles[ch]);
    }

    if(in->hAEC)
        //AecProcessing_Delete(&(in->hAEC));

    aec_dl_sync_flag = 0;   // record mic stop

    enable_gcov();
    ALOGE("%s: exit: status(%d)", __func__, ret);
    return ret;
}
...
int start_input_stream(struct stream_in *in)
{
    ...
    ALOGE("ldx %s: enter: stream(%p)usecase(%d: %s)",
    ...
    if (get_usecase_from_list(adev, usecase) == NULL)
        in->usecase = usecase;
    ALOGE("%s: enter: stream(%p)usecase(%d: %s)",
          __func__, &in->stream, in->usecase, use_case_table[in->usecase]);

    if (CARD_STATUS_OFFLINE == in->card_status||
        CARD_STATUS_OFFLINE == adev->card_status) {
        ALOGE("in->card_status or adev->card_status offline, try again");
        ret = -EIO;
        goto error_config;
    }
    ...
    if (ret)
        ...
        ALOGE("%s: usecase(%d)", __func__, in->usecase);
    ...
    if (in->usecase == USECASE_AUDIO_RECORD_MMAP) {
        ...
        if (audio_extn_ffv_get_stream() == in) {
           ALOGE("%s: ffv stream, update pcm config", __func__);
           audio_extn_ffv_update_pcm_config(&config);
        }
        ALOGE("%s: Opening PCM device card_id(%d) device_id(%d), channels %d",
            ...
            in->pcm_device_id = 0;
            config.rate = 48000;
            config.period_count = 320;
            config.period_size = 960/5;
            config.channels = 16;    // uac only support 2 channel
            config.format = PCM_FORMAT_S16_LE;
#endif


        ALOGE("sdssgl ldx dump config: channels:%d rate:%d format:%d config.period_size:%d config.period_count:%d", config.channels, config.rate, config.format, config.period_size, config.period_count);

        while (1) {
            ATRACE_BEGIN("pcm_in_open");
            ALOGE("sdssgl pcm_in_open before,card(%d,%d).\n",adev->snd_card,in->pcm_device_id);
            in->pcm = pcm_open(adev->snd_card, in->pcm_device_id,
                               flags, &config);
            ATRACE_END();
            if (errno == ENETRESET && !pcm_is_ready(in->pcm)) {
                ALOGE("sdssgl %s: pcm_open failed errno:%d\n", __func__, errno);
            ...
            if (in->pcm == NULL || !pcm_is_ready(in->pcm)) {
                ALOGE("sdssgl %s: %s", __func__, pcm_get_error(in->pcm));
                if (in->pcm != NULL) {
                    pcm_close(in->pcm);
                    in->pcm = NULL;
                }
        ...
        /* for aec dt state file */
        snprintf(in->filename,100,"/data/vendor/dump/aecDTState.txt");
        in->aec_dt_state_file = fopen(in->filename,"wb");

        /* for Aec Output of one channel record */
        snprintf(in->filename,100,"/data/vendor/dump/aec_out_file_1c.pcm");
        in->aec_out_file = fopen(in->filename,"wb");

        /* reserved */
        //snprintf(in->filename,100,"/data/vendor/dump/sdssgl_audio_hal_temp.pcm");
        //in->temp_file = fopen(in->filename,"wb");

        /* for mic pcm in */
        for(int ch = 0; ch < MIC_CHAN_NUM; ch++)
        {
            snprintf(in->filename,100,"/data/vendor/dump/pcm_in_mic%02d.pcm",ch);
            in->chfiles[ch] = fopen(in->filename,"wb");
        }
        /* for ref sig pcm in */
        for(int ch = 0; ch < SPK_CHAN_NUM; ch++)
        {
            snprintf(in->filename,100,"/data/vendor/dump/pcm_in_ref%02d.pcm",ch);
            in->reffiles[ch] = fopen(in->filename,"wb");
        }

    if(1)
    {
        #if (FILE_WRITE == 1)
		//outfiledl = fopen("/sys/devices/system/cpu/present", "r");
        outfiledl = fopen("/data/vendor/dump/dlinput.pcm", "wb");
		if(outfiledl == NULL)
		{
			ALOGE("ldx fopen /data/vendor/dump/dlinput.pcm failed errno:%s\n", strerror(errno));
		}

        outfiledlout = fopen("/data/vendor/dump/dloutput.pcm", "wb");
		if(outfiledlout == NULL)
		ALOGE("ldx fopen /data/vendor/dump/dloutput.pcm failed\n");

#if AEC_DOWNLINK_SYNC
		outfileDTS = fopen("/data/vendor/dump/dlDTState.txt", "aw");
#endif
        #endif
    }
        in->channel_bytes = pcm_format_to_bits(config.format)/8;

        in->buffer_bytes = pcm_frames_to_bytes(in->pcm, pcm_get_buffer_size(in->pcm));
        ALOGE("sdssgl %s: in->buffer_bytes is %d\n", __func__,in->buffer_bytes);
        in->raw_r_buffer = malloc(in->buffer_bytes);    // 960 * 32 bytes
        if(!(in->raw_r_buffer)){

            ALOGE("sdssgl %s: ######## alloc in->raw_r_buffer failed.\n", __func__);
            ret = ENOMEM;
            goto error_open;
        }

        /* damo request: Initialize aec instance */
        AecProcessing_Create_Instance(&(in->hAEC));
		ALOGE("ldx AecProcessing_Create_Instance succed");
        //AecProcessing_Reset(in->hAEC);
...
static audio_usecase_t get_interactive_usecase(struct audio_device *adev)
{
    ...
    ALOGE("%s: num_usecase: %d", __func__, num_usecase);
    ...
    ALOGE("%s: Interactive usecase is %d", __func__, ret_uc);
    return ret_uc;
}
                                 ...
                                 audio_usecase_t uc_id)
{
    ...
    ALOGE("%s: free Interactive usecase %d", __func__, uc_id);
}
...
static audio_usecase_t get_offload_usecase(struct audio_device *adev, bool is_compress)
{
    ...
    ALOGE("%s: offload usecase is %d", __func__, ret_uc);
    return ret_uc;
}
                                 ...
                                 audio_usecase_t uc_id)
{
    ...
    ALOGE("%s: free offload usecase %d", __func__, uc_id);
}
...
static void *offload_thread_loop(void *context)
{
    ...
    for (;;) {
        ...
        if (list_empty(&out->offload_cmd_list)) {
            ALOGE("%s SLEEPING", __func__);
            pthread_cond_wait(&out->offload_cond, &out->lock);
            ALOGE("%s RUNNING", __func__);
            continue;
        }
        ...
        case OFFLOAD_CMD_WAIT_FOR_BUFFER:
            ALOGE("copl(%p):calling compress_wait", out);
            compress_wait(out->compr, -1);
            ALOGE("copl(%p):out of compress_wait", out);
            send_callback = true;
        ...
        case OFFLOAD_CMD_PARTIAL_DRAIN:
            ...
            if(ret == 0) {
                ALOGE("copl(%p):calling compress_partial_drain", out);
                ret = compress_partial_drain(out->compr);
                ALOGE("copl(%p):out of compress_partial_drain", out);
                if (ret < 0)
                    ret = -errno;
            }
            ...
            if (-ENETRESET != ret && !(-EINTR == ret &&
                ...
                ALOGE("copl(%p):send drain callback, ret %d", out, ret);
            } else
                ALOGE("%s: Block drain ready event during SSR", __func__);
            break;
        case OFFLOAD_CMD_DRAIN:
            ALOGE("copl(%p):calling compress_drain", out);
            ret = compress_drain(out->compr);
            ALOGE("copl(%p):out of compress_drain", out);
        ...
        case OFFLOAD_CMD_ERROR:
            ALOGE("copl(%p): sending error callback to AF", out);
            send_callback = true;
...
static int stop_output_stream(struct stream_out *out)
{
    ...
    if (is_offload_usecase(out->usecase) &&
        (audio_extn_passthru_is_passthrough_stream(out))) {
        ALOGE("Disable passthrough , reset mixer to pcm");
        /* NO_PASSTHROUGH */
    ...
    if (has_voip_usecase ||
        ...
        list_for_each(node, &adev->usecase_list) {
            ...
            ALOGE("%s: select_devices at usecase(%d: %s) after removing the usecase(%d: %s)",
    ...
    free(uc_info);
	ALOGE("ldx DLProcessing_Delete(hDL)");
	DLProcessing_Delete(hDL);
    ALOGE("%s: exit: status(%d)", __func__, ret);
    return ret;
}
...
#define IPC_DAMO 0x12344321
#define FOUND_CARD_INDEX
...
int start_output_stream(struct stream_out *out)
{
    ...
    for(i = 0; i<3; i++)
    {
        sprintf(str, "/proc/asound/card%d/id",i);
        fd = open(str,O_RDONLY);
        if(fd < 0)
            ALOGE("ldx fail to open:%s\n", str);

        retval = read(fd, name, 3);
        if(retval < 0)
        {
            ALOGE("ldx read failed\n");
            continue;
        }

        retval = strncmp("x20", name, 3);
        ALOGE("ldx name of card%d: %s retval:%d\n", i, name, retval);
        close(fd);
    ...
    cardindex = i;
    ALOGE("ldx found card idx:%d\n", cardindex);
	//system("mkdir /data/vendor/dump");
	//system("chmod 777 /data/vendor/dump -R");
    #endif
    ...
    ALOGE("%s: enter: stream(%p)usecase(%d: %s) devices(%#x) is_haptic_usecase(%d)",
          __func__, &out->stream, out->usecase, use_case_table[out->usecase],
          out->devices, is_haptic_usecase);

    if (CARD_STATUS_OFFLINE == out->card_status ||
        CARD_STATUS_OFFLINE == adev->card_status) {
        ALOGE("out->card_status or adev->card_status offline, try again");
        ret = -EIO;
        goto error_config;
    }
    ...
    if (out->devices & AUDIO_DEVICE_OUT_ALL_SCO) {
        if (!adev->bt_sco_on) {
            if (out->devices & AUDIO_DEVICE_OUT_SPEAKER) {
                ...
                ALOGE("%s: SCO is not connected, route it to speaker", __func__);
    ...
    ALOGE("%s: Opening PCM device card_id(%d) device_id(%d) format(%#x)",
          __func__, adev->snd_card, out->pcm_device_id, out->config.format);

    if (out->usecase == USECASE_AUDIO_PLAYBACK_MMAP) {
        ALOGE("%s: Starting MMAP stream", __func__);
        ...
        if ((adev->vr_audio_mode_enabled) &&
            ...
            ctl = mixer_get_ctl_by_name(adev->mixer, mixer_ctl_name);
            if (!ctl) {
                ALOGE("%s: Could not get ctl for mixer cmd might be ULL - %s",
                      __func__, mixer_ctl_name);
            } else {
                //if success use ULLPP
                ALOGE("%s: mixer ctrl %s succeeded setting up ULL for %d",
        ...
        //ALOGE("ldx line:%d dump pcm_write: out->config.period_size:%d, out->config.period_count:%d out->config.channels:%d out->config.rate:%d", __LINE__, out->config.period_size, out->config.period_count, out->config.channels, out->config.rate);
        adev->snd_card = cardindex;
        out->pcm_device_id = 0;

		DLProcessing_Create_Instance(&hDL);
        ...
        if (is_haptic_usecase) {
            ...
            if (property_get_bool("vendor.audio.enable_haptic_audio_sync", false)) {
                ALOGE("%s: enable haptic audio synchronization", __func__);
                            ...
                            int duration /*in millisecs*/)
{
    ...
    if ((duration == 0) || (sample_rate == 0) ||
        ...
        ALOGE("Invalid config duration %d sr %d bps %d ch %d", duration, sample_rate,
...
static int out_standby(struct audio_stream *stream)
{
    ...
    ALOGE("%s: enter: stream (%p) usecase(%d: %s)", __func__,
    ...
    if (!out->standby) {
        ...
        if (out->usecase == USECASE_COMPRESS_VOIP_CALL) {
            ...
            pthread_mutex_unlock(&adev->lock);
            pthread_mutex_unlock(&out->lock);
            ALOGE("VOIP output entered standby");
            return 0;
        } else if (!is_offload_usecase(out->usecase)) {
            if (out->pcm) {
                pcm_close(out->pcm);
...
static int out_on_error(struct audio_stream *stream)
{
    ...
    if (is_offload_usecase(out->usecase) && out->card_status == CARD_STATUS_OFFLINE) {
        ALOGE("Setting previous card status if offline");
...
static int parse_compress_metadata(struct stream_out *out, struct str_parms *parms)
{
    ...
    ALOGE("%s new encoder delay %u and padding %u", __func__,
...
static void out_snd_mon_cb(void * stream, struct str_parms * parms)
{
    ...
    if (status == CARD_STATUS_OFFLINE) {
        ...
        if (voice_is_call_state_active(adev) &&
            ...
            ALOGE("%s: SSR/PDR occurred, end all calls\n", __func__);
            pthread_mutex_lock(&adev->lock);
...
static int out_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
    ...
    if (err >= 0) {
        ...
        */
        ...
        if (audio_is_usb_out_device(new_dev) &&
            ...
            ALOGE("out_set_parameters() ignoring rerouting to non existing USB card %d", card);
            pthread_mutex_unlock(&adev->lock);
            pthread_mutex_unlock(&out->lock);
            ret = -ENOSYS;
            goto routing_fail;
                    ...
                    if (adev->mode == AUDIO_MODE_IN_CALL) {
                        ...
                        if (audio_is_usb_out_device(out->devices & AUDIO_DEVICE_OUT_ALL_USB)) {
                            ...
                            audio_extn_usb_set_service_interval(true /*playback*/,
                                                                service_interval,
                                                                &reconfig);
                            ALOGE("%s, svc_int(%ld),reconfig(%d)",__func__,service_interval, reconfig);
                         }
    ...
    if (err >= 0) {
        ...
        ALOGE("updating stream profile with value '%s'", out->profile);
        lock_output_stream(out);
    ...
    if (out->hal_output_suspend_supported && out->dynamic_pm_qos_config_supported) {
        ...
        if (str_parms_get_str(parms, "suspend_playback", value, sizeof(value)) >= 0) {
            ALOGE("%s: got suspend_playback %s", __func__, value);
            lock_output_stream(out);
    ...
    str_parms_destroy(parms);
error:
    ALOGE("%s: exit: code(%d)", __func__, ret);
    return ret;
}
...
static char* out_get_parameters(const struct audio_stream *stream, const char *keys)
{
    ...
    ALOGE("%s: %s enter: keys - %s", __func__, use_case_table[out->usecase], keys);
    ...
    str_parms_destroy(query);
    str_parms_destroy(reply);
    ALOGE("%s: exit: returns - %s", __func__, str);
    return str;
}
...
static uint32_t out_get_latency(const struct audio_stream_out *stream)
{
    ...
    ALOGE("%s: Latency %d", __func__, latency);
    return latency;
}
                          ...
                          float right)
{
    ...
    if (left != right)
        ALOGE("%s: Left and right channel volume mismatch:%f,%f",
                 __func__, left, right);
                              ...
                              float right)
{
    ...
    if (left != right) {
        ...
        ALOGE("%s : Pcm set volume value %d left %f", __func__, volume, left);
                          ...
                          float right)
{
    ...
    ALOGE("%s: called with left_vol=%f, right_vol=%f", __func__, left, right);
    if (out->usecase == USECASE_AUDIO_PLAYBACK_MULTI_CH) {
        ...
        if (audio_extn_passthru_is_passthrough_stream(out)) {
            ...
            ALOGE("%s: Overriding offload set volume for media bus stream", __func__);
            struct listnode *node = NULL;
            list_for_each(node, &adev->active_outputs_list) {
                streams_output_ctxt_t *out_ctxt = node_to_item(node,
                                                    streams_output_ctxt_t,
        ...
        ALOGE("%s: MMAP set volume called", __func__);
        if (!out->standby)
        ...
        ALOGE("%s: bus device set volume called", __func__);
        if (!out->standby)
            ret = out_set_pcm_volume(stream, left, right);
        out->volume_l = left;
        out->volume_r = right;
                         ...
                         size_t bytes)
{
    ...
    int loop_count = 0;
    int loop_index = 0;
    short int DL_input_48k[FRAME_20ms_48];
	short int DL_output_48k[FRAME_20ms_48];
    //void *hDL;
    short *buffer_temp = (short *)buffer;
    char *buffer_temp_write = (void *)buffer;
    //short *buffer_dl = (short *)malloc(bytes);
    //void *buffer_dl_temp = (void *)buffer_dl;
    //char *buffer_dl_write = buffer_dl_temp;

    static int sumindex = 0;
    static char *sumbuffer = &array_sumbytes[0];
    static int   sumbytes = 0;
    static int   buffercount = 0;
    static int   finish_sum = 0;
    ...
    lock_output_stream(out);
    //ALOGE("ldx out_write: %zu bytes sizeof(size_t):%u\n", bytes, (unsigned int)sizeof(size_t));
    if (CARD_STATUS_OFFLINE == out->card_status) {

        if (out->flags & AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD) {
            /*during SSR for compress usecase we should return error to flinger*/
            ALOGE(" copl %s: sound card is not active/SSR state", __func__);
            pthread_mutex_unlock(&out->lock);
            ATRACE_END();
            return -ENETRESET;
        } else {
            ALOGE(" %s: sound card is not active/SSR state", __func__);
            ret= -EIO;
            goto exit;
        }
    }
    ...
    if (audio_extn_passthru_should_drop_data(out)) {
        ALOGE(" %s : Drop data as compress passthrough session is going on", __func__);
        ret = -EIO;
        goto exit;
    }
               ...
               (out->format == AUDIO_FORMAT_DTS_HD)) {
                ...
                if (ret) {
                    if (ret != -ENOSYS) {
                        ...
                        ALOGE("iec61937 transmission info not yet updated retry");
                    }
                } else if (!out->standby) {
    ...
    if (out->standby) {
        ...
        if (out->usecase == USECASE_COMPRESS_VOIP_CALL)
        {
			...
			//ALOGE("ldx %s: line:%d ret:%d", __func__, __LINE__, (int)ret);
		}
        else
        {
        ...
        if (last_known_cal_step != -1) {
            ALOGE("%s: retry previous failed cal level set", __func__);
    ...
    if (is_offload_usecase(out->usecase)) {
        ...
        if (out->send_new_metadata) {
            ALOGE("copl(%p):send new gapless metadata", out);
            compress_set_gapless_metadata(out->compr, &out->gapless_mdata);
            out->send_new_metadata = 0;
            if (out->send_next_track_params && out->is_compr_metadata_avail) {
                ALOGE("copl(%p):send next track params in gapless", out);
        ...
        if (!(out->flags & AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD) &&
            ...
            if ((bytes > out->hal_fragment_size)) {
                ALOGE("Error written bytes %zu > %d (fragment_size)",
        ...
        if (ret >= 0 && ret < (ssize_t)bytes && out->non_blocking) {
            ALOGE("No space available in compress driver, post msg to cb thread");
        ...
        if (out->pcm) {
            ...
            //ALOGE("%s: frames=%zu, frame_size=%zu, bytes_to_write=%zu",
            ...
            if (use_mmap)
                ...
                if ((out->flags & AUDIO_OUTPUT_FLAG_VOIP_RX) &&
                    ...
                    {    
					    if(bytes == 768)
                        {
                            //ALOGE("sdssgl use downlink.\n");
		                if(bytes == 768)
		                {
		                    
		                    {
								//ALOGE("ldx sumbuffer:%p\n", sumbuffer);
		                        memcpy(sumbuffer, buffer, bytes);
		                        sumbuffer = sumbuffer + bytes;
		                        sumbytes += bytes;
		                        sumindex = sumindex + 1;
		                    }
		                    if(sumindex == 10)
		                    {
		                        //ALOGE("now we got enough buffer to proceed in downlink so");
		                        finish_sum = 1;
		                        sumindex = 0;
		                    }    
		                    else
		                    {
		                        ret = 0;
		                        finish_sum = 0;
		                        //ALOGE("not enough buffer to proceed, we jump out to wait :%d bytes sumbytes:%d", sumindex, sumbytes);
		                        goto exit;
		                    }    
			            ...
			            if(finish_sum == 1)
			            {
							//ALOGE("finish_sum %d sumbytes:%d now we got enough buffer to proceed in downlink so", finish_sum, sumbytes);
							buffer_temp = (short *)&array_sumbytes[0];
							buffer_temp_write = (char *)&array_sumbytes[0];
							//ALOGE("ldx buffer_temp:%p buffer_temp:%p\n", buffer_temp, buffer_temp_write);
							buffercount = sumbytes;
                            if(file_write_flags)
                            {
							    //fwrite(buffer_temp_write, 1, buffercount, outfiledl);
                            }

							loop_count = (sumbytes/FRAME_20ms_48/4);
							sumbuffer  = (void *)&array_sumbytes[0];
							sumbytes = 0;
							finish_sum = 0;
			            }    
			            else
			            {
                            if(file_write_flags)
                            {
							    //fwrite(buffer_temp_write, 1, bytes, outfiledl);
                            }
							loop_count = (bytes/FRAME_20ms_48/4); 
						}
						
								                //ALOGE("ldx loop_count:%d buffer_temp addr:%p\n", loop_count, buffer_temp);
		                for(loop_index = 0; loop_index < loop_count; loop_index++)
		                {
		                    //ALOGE("ldx trace %d uffer_temp:%p", __LINE__, buffer_temp);
		                    /*取左声道0,丢弃右声道1*/
		                    for (i = 0; i < FRAME_20ms_48; i++)
		                    {
		                        DL_input_48k[i] = (short)buffer_temp[2*i];
                            ...
                            if(file_write_flags)
                            {
                                ...
                                memset(wr_buf,' ',32);
                                n = sprintf(wr_buf, "%ld.%ld dlDTState is %d\n",time_start.tv_sec,time_start.tv_nsec, g_dt_state);
                                wr_buf[n] = '\0';
							    fwrite(wr_buf, 1, 32, outfileDTS);
                            }
							#endif
							int alc = 0;
							DLProcessing_SetParam(ALC_FLAG, alc, hDL);
							DLProcessing_SetParam(DT_STATE, g_dt_state, hDL);

		                    DLProcessing_FrameProcess(
		                      DL_input_48k, 
		                      DL_output_48k,
		                      hDL
		                    );
					if(is_first_flag_dl)
					{
						...
						//unsigned int dl_ver = DLProcessing_GetLibVersion();
                        unsigned int dl_ver = 0x1234;
						ALOGI("Downlink Source Code Version is %d.%d.%d\n",((dl_ver >> 16) & 0xff),((dl_ver >> 8) & 0xff),(dl_ver & 0xff));
						is_first_flag_dl = 0;

                        memset(value,'\0',PROPERTY_VALUE_MAX);
                        snprintf(value,10,"%d.%d.%d",(dl_ver >> 16) & 0xff,(dl_ver >> 8) & 0xff,dl_ver & 0xff);
                        property_set("downlink.source.code.version",value);
					}	

                            if(file_write_flags)
                            {
							    fwrite(DL_input_48k, 1, FRAME_20ms_48*2, outfiledl);
						        fwrite(DL_output_48k, 1, FRAME_20ms_48*2, outfiledlout);
                            }

		                    //ALOGE("ldx trace %d", __LINE__);
		                    /*把处理后的左声道数据同时复制到左右两个声道*/
		                    for (i = 0; i < FRAME_20ms_48; i++)
		                    {
		                       buffer_temp[2*i] = (short) DL_output_48k[i];
		                       buffer_temp[(2*i)+1] = (short) DL_output_48k[i]; 
		                    }  
		                  
		                    buffer_temp += FRAME_20ms_48 * 2; 
		                    //buffer_dl  += FRAME_20ms_48 * 2;
		                }
						...
						if(buffercount != 0)
						{
                            if(file_write_flags)
                            {
						        //fwrite(buffer_temp_write, 1, buffercount, outfiledlout);
                            }
							
							#if 0
							for(write_index = 0; write_index < (buffercount/bytes); write_index++)
							{
								ret = pcm_write(out->pcm, (void *)buffer_temp_write, bytes);
								ALOGE("ldx trace %d bytes:%zu @addr:%p\n", __LINE__, bytes*(write_index+1),buffer_temp_write);
								buffer_temp_write += bytes;

							}
							buffer_temp_write = (char *)&array_sumbytes[0];
							#endif

							out->config.period_size = 1920;
							ret = pcm_write(out->pcm, (void *)buffer_temp_write, buffercount);
							memset(buffer_temp_write, 0, buffercount);
							buffercount = 0;
						...
						}else
						{
                            ...
                            if(file_write_flags)
                            {
						        //fwrite(buffer_temp_write, 1, bytes, outfiledlout);
                            }
							memset(buffer_temp_write, 0, bytes);
						}	
                    }else{
                        ALOGE("sdssgl not use downlink.\n");
            ...
            stream_callback_t callback, void *cookie)
{
    ...
    if (out->adsp_hdlr_stream_handle) {
        ret = audio_extn_adsp_hdlr_stream_set_callback(
                                ...
                                cookie);
        if (ret)
            ALOGE("%s:adsp hdlr callback registration failed %d",
                   __func__, ret);
    }
    pthread_mutex_unlock(&out->lock);
    return 0;
...
static int out_pause(struct audio_stream_out* stream)
{
    ...
    ALOGE("%s", __func__);
    if (is_offload_usecase(out->usecase)) {
        ALOGE("copl(%p):pause compress driver", out);
        lock_output_stream(out);
        if (out->compr != NULL && out->offload_state == OFFLOAD_STATE_PLAYING) {
            ...
            if (audio_extn_passthru_is_active()) {
                ALOGE("offload use case, pause passthru");
...
static int out_resume(struct audio_stream_out* stream)
{
    ...
    ALOGE("%s", __func__);
    if (is_offload_usecase(out->usecase)) {
        ALOGE("copl(%p):resume compress driver", out);
        status = 0;
        lock_output_stream(out);
...
static int out_flush(struct audio_stream_out* stream)
{
    ...
    ALOGE("%s", __func__);
    if (is_offload_usecase(out->usecase)) {
        ALOGE("copl(%p):calling compress flush", out);
        lock_output_stream(out);
        if (out->offload_state == OFFLOAD_STATE_PAUSED) {
            stop_compressed_output_l(out);
        } else {
            ALOGE("%s called in invalid state %d", __func__, out->offload_state);
        }
        out->written = 0;
        pthread_mutex_unlock(&out->lock);
        ALOGE("copl(%p):out of compress flush", out);
        return 0;
    }
    return -ENOSYS;
}
...
static void adjust_mmap_period_count(struct pcm_config *config, int32_t min_size_frames)
{
    ...
    ALOGE("%s original config.period_size = %d config.period_count = %d",
    ...
    ALOGE("%s requested config.period_count = %d", __func__, config->period_count);
}
...
static int64_t get_mmap_out_time_offset() {
    ...
    ALOGE("mmap_time_offset_micros = %d for output", mmap_time_offset_micros);
                                  ...
                                  struct audio_mmap_buffer_info *info)
{
    ...
    ALOGE("%s: got mmap buffer address %p info->buffer_size_frames %d",
...
static int in_standby(struct audio_stream *stream)
{
    ...
    if (!in->standby && in->is_st_session) {
        ALOGE("%s: sound trigger pcm stop lab", __func__);
    ...
    if (!in->standby) {
        ...
        if (in->usecase == USECASE_COMPRESS_VOIP_CALL) {
            ...
            ALOGE("VOIP input entered standby");
...
static void in_snd_mon_cb(void * stream, struct str_parms * parms)
{
    ...
    ALOGE("in_snd_mon_cb for card %d usecase %s, status %s", card,
...
static int in_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
    ...
    ALOGE("%s: enter: kvpairs=%s", __func__, kvpairs);
    parms = str_parms_create_str(kvpairs);
    ...
    if (err >= 0) {
        ...
        if (((int)in->device != val) && (val != 0) && audio_is_input_device(val) ) {
            ...
            if (audio_is_usb_in_device(val) &&
                ...
                ALOGE("in_set_parameters() ignoring rerouting to non existing USB card %d", card);
                ret = -ENOSYS;
            } else {
                ...
                if (!in->standby && !in->is_st_session) {
                    ALOGE("update input routing change");
    ...
    if (err >= 0) {
        ...
        ALOGE("updating stream profile with value '%s'", in->profile);
                               ...
                               const char *keys)
{
    ...
    ALOGE("%s: enter: keys - %s %s ", __func__, use_case_table[in->usecase], keys);
...
static unsigned long pre_MD_Control_Para = -1;
static unsigned long MD_Control_Para = -1;
static unsigned long pre_MD_Debug_Para = -1;
static unsigned long MD_Debug_Para = -1;
static unsigned int MD_Debug_Para_UINT = -1;
                       ...
                       size_t bytes)
{
    ...
    if (in->standby) {
        ...
        if (in->usecase == USECASE_COMPRESS_VOIP_CALL)
		{
			...
			ALOGE("ldx %s: line:%d ret:%d", __func__, __LINE__, ret);
		}
        else
        {
    ...
    if (android_atomic_acquire_load(&(in->capture_stopped)) > 0) {
        ALOGE("%s: force stopped catpure session, ignoring read request", __func__);
        ret = -EINVAL;
        goto exit;
    }
    ...
    if (audio_extn_cin_attached_usecase(in->usecase)) {
        ...
        if (audio_extn_ssr_get_stream() == in) {
            ...
            memset(in->raw_r_buffer,0,in->buffer_bytes);
            //bytes = in->buffer_bytes;
            bytes = 30720;
            ret = pcm_read(in->pcm, in->raw_r_buffer, bytes);
            /* data from DSP comes in 24_8 format, convert it to 8_24 */
            if (!ret && bytes > 0 && (in->format == AUDIO_FORMAT_PCM_8_24_BIT)) {
                if (audio_extn_utils_convert_format_24_8_to_8_24(in->raw_r_buffer, bytes)
                    != bytes) {
                    ret = -EINVAL;
                    goto exit;
                }
        ...
        memset(in->s_MicBuff,0,sizeof(in->s_MicBuff));
        for(int i = 0;i < FRAME_20ms_48 ;i++)
        {
            for(int j = 0 ; j < MIC_ARRAY_CHANNELS ; j++)
            {
                //sort the mic data
                memcpy(&((in->s_MicBuff)[j][i]),&(((short *)(in->raw_r_buffer) + i*MIC_ARRAY_CHANNELS)[j]),in->channel_bytes); 
            }
        }
#if 0
        /* for mic record ,test ok*/
        for(int i = 0; i < MIC_CHAN_NUM; i++)
        {
            if(file_write_flags)
            {
                fwrite(&(in->s_MicBuff[i][0]),in->channel_bytes,FRAME_20ms_48,in->chfiles[i]);
                
            }
        }
#endif

        for(int MicNum = 0; MicNum < MIC_CHAN_NUM; MicNum++)
        {
            /* pointer Mic channel */
            (in->pMicPCM)[MicNum] = &((in->s_MicBuff)[MicNum][0]);

            if(file_write_flags)
            {
                /* for Mic Channel record */
                fwrite(in->pMicPCM[MicNum],in->channel_bytes,FRAME_20ms_48,in->chfiles[MicNum]);

            }
        }

        /* channel 12:ref mic,13:speaker mic */
        for(int ref_index = 0 ; ref_index < SPK_CHAN_NUM; ref_index++)
        {
            (in->Speaker_Mic_Ref)[1 - ref_index] = &((in->s_MicBuff)[10 + ref_index + 1][0]); 
        }

        for(int i = 0; i < SPK_CHAN_NUM; i++)
        {
            if(file_write_flags)
            {
                // test ok
                fwrite((in->Speaker_Mic_Ref)[i],in->channel_bytes,FRAME_20ms_48,in->reffiles[i]);
            
            }
        }


        clock_gettime(CLOCK_MONOTONIC, &(in->starttime));

        /* for damo midi control */
        memset(value,'\0',PROPERTY_VALUE_MAX);
        if(property_get("adb.mdctrl",value,NULL) > 0)
        {
            MD_Control_Para = strtoul(value,NULL,16);
        }


        if(MD_Control_Para != pre_MD_Control_Para)
        {
            //AecProcessing_SetAllEnableFlags(MD_Control_Para,in->hAEC);
            ALOGI("sdssgl MD_Control_para is %ld, PROPERTY_VALUE_MAX is %d.\n",MD_Control_Para,PROPERTY_VALUE_MAX);
            pre_MD_Control_Para = MD_Control_Para;
        }

        /* for damo midi debug */
        memset(value,'\0',PROPERTY_VALUE_MAX);
        if(property_get("adb.mddebug",value,NULL) > 0)
        {
            MD_Debug_Para = strtoul(value,NULL,16);
        }

        if(MD_Debug_Para != pre_MD_Debug_Para)
        {
            MD_Debug_Para_UINT = (unsigned int)(MD_Debug_Para);
            ALOGI("sdssgl MD_Debug_para is 0x%x.\n",MD_Debug_Para_UINT);
            //AecProcessing_SetDebugMask(MD_Debug_Para_UINT,in->hAEC);
            pre_MD_Debug_Para = MD_Debug_Para;
        }

        /* for damo midi pcm dump open/close */
        memset(value,'\0',PROPERTY_VALUE_MAX);
        if(property_get("adb.mddump",value,NULL) > 0)
        {
            file_write_flags = strtoul(value,NULL,16);
            file_write_flags = !!file_write_flags;
        }

        AecProcessing_FrameProcess(
           in->pMicPCM,
           in->Speaker_Mic_Ref[0],
           in->Speaker_Mic_Ref[1],
           buffer,
           in->hAEC);

#if 0
        memset(value,'\0',PROPERTY_VALUE_MAX);
        int get_nrtype = AecProcessing_GetEnableFlag(VAD_NOISE_SUPPRESSION,in->hAEC);
        snprintf(value,10,"%d",get_nrtype);
        property_set("adb.mdused",value);
#endif

        if(is_first_flag_aec)
        {
            //unsigned int aec_ver = AecProcessing_GetLibVersion();
            unsigned int aec_ver = 0x1234;
            ALOGI("MIDI Source Code Version is %d.%d.%d\n",((aec_ver >> 16) & 0xff),((aec_ver >> 8) & 0xff),(aec_ver & 0xff));
            is_first_flag_aec = 0;

            memset(value,'\0',PROPERTY_VALUE_MAX);
            snprintf(value,10,"%d.%d.%d",(aec_ver >> 16) & 0xff,(aec_ver >> 8) & 0xff,aec_ver & 0xff);
            property_set("midi.source.code.version",value);
        }	

        clock_gettime(CLOCK_MONOTONIC, &(in->endtime));
        in->suanfatime = ((in->endtime.tv_sec - in->starttime.tv_sec) * 1000000) + (in->endtime.tv_nsec - in->starttime.tv_nsec)/1000;


       //int DTState = AecProcessing_GetDTState(in->hAEC);
       int DTState = 0;
       ...
       g_dt_state = DTState;
        ...
        if(file_write_flags)
        {
            ...
            clock_gettime(CLOCK_MONOTONIC, &(in->starttime));
            memset(wr_buf,' ',32);
            n = sprintf(wr_buf, "%ld.%ld aecDTState is %d\n",in->starttime.tv_sec,(in->starttime.tv_nsec), DTState);
            wr_buf[n] = '\0';

            //ALOGE("sdssgl %s AecProcessing_FrameProcess alg time is %ld us,state is %d.\n", __func__,in->suanfatime,DTState);
            fwrite(wr_buf,1,32,in->aec_dt_state_file);
            fwrite(buffer,in->channel_bytes,FRAME_20ms_48,in->aec_out_file);
        }


        /* bytes read is always set to bytes for non compress usecases */
        bytes_read = 1920;  // android read 1 channel,960 frames,2 bytes per frames
    }
...
exit:
    ...
    if (ret != 0) {
        ...
        ALOGE("%s: read failed status %d- sleeping for buffer duration", __func__, ret);
                            ...
                            struct listnode *head)
{
    ...
    if (add) {
        if (target) {
            ALOGE("effect %p already exist", effect);
            return ret;
        }
                                   ...
                                   bool enable)
{
    ...
    ALOGE("%s: status %d in->standby %d enable:%d", __func__, status, in->standby, enable);
...
static int in_add_audio_effect(const struct audio_stream *stream,
                               effect_handle_t effect)
{
    ALOGE("%s: effect %p", __func__, effect);
...
static int in_start(const struct audio_stream_in* stream)
{
	...
	ALOGE("ldx %s: line:%d ret:%d in-usecase:%d", __func__, __LINE__, ret, in->usecase);
...
static int64_t in_get_mmap_time_offset() {
    ...
    ALOGE("mmap_time_offset_micros = %d for input", mmap_time_offset_micros);
    return mmap_time_offset_micros * (int64_t)1000;
}
...
static void in_update_sink_metadata(struct audio_stream_in *stream,
    ...
    lock_input_stream(in);
    pthread_mutex_lock(&adev->lock);
    ALOGE("%s: in->usecase: %d, device: %x", __func__, in->usecase, device);
                            ...
                            const char *address)
{
	...
	//ALOGE("ldx forece set is_voip_rx = false");
	//is_voip_rx = false;

    if (is_usb_dev && (!audio_extn_usb_connected(NULL))) {
        is_usb_dev = false;
        devices = AUDIO_DEVICE_OUT_SPEAKER;
        ALOGE("%s: ignore set device to non existing USB card, use output device(%#x)",
              __func__, devices);
    }
    ...
    *stream_out = NULL;
    ...
    if (out_get_stream(adev, handle) != NULL) {
        ALOGE("%s, output stream already opened", __func__);
        ret = -EEXIST;
    }
    pthread_mutex_unlock(&adev->lock);
    if (ret)
    ...
    ALOGE("%s: enter: format(%#x) sample_rate(%d) channel_mask(%#x) devices(%#x) flags(%#x)\
    ...
    if (direct_dev &&
        ...
        pthread_mutex_lock(&adev->lock);
        if (is_hdmi) {
            ALOGE("AUDIO_DEVICE_OUT_AUX_DIGITAL and DIRECT|OFFLOAD, check hdmi caps");
            ...
            ALOGE("plugged dev USB ret %d", ret);
       }
    ...
    if (out->devices & AUDIO_DEVICE_OUT_BUS) {
        ...
        ALOGE("%s: address %s, car_audio_stream %x",
    ...
    if (is_voip_rx) {
            ...
            if (out->sample_rate == 8000 || out->sample_rate == 16000 ||
             out->sample_rate == 32000 || out->sample_rate == 48000) {
                ...
                if (frame_size != 0)
                    ...
                    ALOGE("%s: frame size is 0 for format %#x", __func__, out->format);
            }
        } else {
        ...
        if (out->flags & AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD) {
            ...
            ALOGE("Compress Offload usecase .. usecase selected %d", out->usecase);
        } else {
            out->usecase = get_offload_usecase(adev, false /* is_compress */);
            ALOGE("non-offload DIRECT_usecase ... usecase selected %d ", out->usecase);
        }

        if (out->flags & AUDIO_OUTPUT_FLAG_FAST) {
            ALOGE("%s: Setting latency mode to true", __func__);
        ...
        if (out->usecase == USECASE_INVALID) {
            if (out->devices & AUDIO_DEVICE_OUT_AUX_DIGITAL &&
                ...
                ALOGE("%s dummy open to query sink capability",__func__);
        ...
        ALOGE("%s: offloaded output offload_info version %04x bit rate %d",
        ...
        if (out->flags & AUDIO_OUTPUT_FLAG_INTERACTIVE) {
            ...
            if (!out->dynamic_pm_qos_config_supported) {
                ALOGE("%s: dynamic qos voting not enabled for platform", __func__);
            } else {
                ALOGE("%s: dynamic qos voting enabled for platform", __func__);
                ...
                ALOGE("%s: created %s pm_qos_mixer_path" , __func__,
        ...
        if (out->hal_ip_format != out->hal_op_format) {
            ...
            ALOGE("Convert buffer allocated of size %d", buffer_size);
        }
    }

    ALOGE("%s devices:%d, format:%x, out->sample_rate:%d,out->bit_width:%d out->format:%d out->flags:%x, flags: %x usecase %d",
    ...
    *stream_out = &out->stream;
    ALOGE("%s: Stream (%p) picks up usecase (%s)", __func__, &out->stream,
           use_case_table[out->usecase]);
                                     ...
                                     struct audio_stream_out *stream)
{
    ...
    ALOGE("%s: enter:stream_handle(%s)",__func__, use_case_table[out->usecase]);
    ...
    if (out_ctxt != NULL) {
        ...
        ALOGE("%s, output stream already closed", __func__);
    }
    free(stream);
    pthread_mutex_unlock(&adev->lock);
    ALOGE("%s: exit", __func__);
...
static int adev_set_parameters(struct audio_hw_device *dev, const char *kvpairs)
{
    ...
    ALOGE("%s: enter: %s", __func__, kvpairs);
    parms = str_parms_create_str(kvpairs);
        ...
        if (!strncmp(value, "false", 5) &&
            ...
            list_for_each(node, &adev->usecase_list) {
                ...
                if (usecase->stream.in && (usecase->type == PCM_CAPTURE) &&
                    ...
                    ALOGE("a2dp resumed, switch bt sco mic to handset mic");
    ...
    if (ret >= 0) {
        ...
        if (audio_is_output_device(val) &&
            ...
            ALOGE("cache new ext disp type and edid");
            ...
            if (!audio_extn_usb_is_tunnel_supported()) {
                ALOGE("detected USB connect .. disable proxy");
    ...
    if (ret >= 0) {
        ...
        if (audio_is_usb_out_device(device) || audio_is_usb_in_device(device)) {
            ...
            if (!audio_extn_usb_is_tunnel_supported()) {
                ALOGE("detected USB disconnect .. enable proxy");
    ...
    if (status >= 0 && a2dp_reconfig) {
        ...
        list_for_each(node, &adev->usecase_list) {
            ...
            if (usecase->stream.out && (usecase->type == PCM_PLAYBACK) &&
                (usecase->devices & AUDIO_DEVICE_OUT_ALL_A2DP)){
                ALOGE("reconfigure a2dp... forcing device switch");
                pthread_mutex_unlock(&adev->lock);
                lock_output_stream(usecase->stream.out);
                pthread_mutex_lock(&adev->lock);
                audio_extn_a2dp_set_handoff_mode(true);
                ALOGE("Switching to speaker and muting the stream before select_devices");
                check_a2dp_restore_l(adev, usecase->stream.out, false);
                //force device switch to re configure encoder
                select_devices(adev, usecase->id);
                ALOGE("Unmuting the stream after select_devices");
    ...
    ret = str_parms_get_str(parms, AUDIO_PARAMETER_KEY_VR_AUDIO_MODE,
        value, sizeof(value));
    if (ret >= 0) {
        ALOGE("Setting vr mode to be %s", value);
        if (!strncmp(value, "true", 4)) {
            adev->vr_audio_mode_enabled = true;
            ALOGE("Setting vr mode to true");
        } else if (!strncmp(value, "false", 5)) {
            adev->vr_audio_mode_enabled = false;
            ALOGE("Setting vr mode to false");
        } else {
            ALOGE("wrong vr mode set");
        }
    }
    ...
    if (ret >= 0) {
        ...
        if (strcmp(value, AUDIO_PARAMETER_VALUE_FRONT) == 0)
            ...
            ALOGE("%s: invalid camera facing value: %s", __func__, value);
            goto done;
        }
        adev->camera_orientation =
    ...
    pthread_mutex_unlock(&adev->lock);
error:
    ALOGE("%s: exit with code(%d)", __func__, status);
    return status;
}
                                 ...
                                 const char *keys)
{
    ...
    if (ret >= 0) {
        ...
        ALOGE("getting vr mode to %d", vr_audio_enabled);
...
static int mw_set_led_status(int status)
{
    int fd = -1;
    char buf[2] = {0};

    fd = open("/sys/class/leds/lcd-backlight/mw_led_status",O_RDWR);
    if(fd < 0)
    {
        ALOGE("sdssgl %s open mw_led_status file failed.\n", __func__);
    }
    ...
    if(status){
        ...
        buf[1] = '\0';
        write(fd,buf,2); 
        ALOGE("sdssgl %s write status is %d.\n",__func__,status);
    }else{
        buf[0] = '0';
        buf[1] = '\0';
        write(fd,buf,2); 
...
static int adev_set_mic_mute(struct audio_hw_device *dev, bool state)
{
    ...
    if(true == state)
    {
        ALOGE("sdssgl %s state %d.\n", __func__, state);
        mw_set_led_status(1);
    }else{
        //system("echo 0 > /sys/class/leds/lcd-backlight/mw_led_status");
        ALOGE("sdssgl %s state %d.\n", __func__, state);
        mw_set_led_status(0);
    }
                                               ...
                                               struct audio_config *config)
{
    ...
    if(!voice_extn_is_compress_voip_supported()) {
        ...
        in->config.period_size = VOIP_IO_BUF_SIZE(in->sample_rate,
            ...
            ALOGE("%s No valid input in voip, use defaults"
        ...
        if ((in->dev->mode == AUDIO_MODE_IN_COMMUNICATION ||
            ...
            ALOGE("%s compress voip not active, use defaults", __func__);
        }
    }
    return 0;
}
                                  ...
                                  audio_source_t source)
{
    ...
    bool may_use_hifi_record = adev_input_allow_hifi_record(adev,
                                                            ...
                                                            flags,
                                                            source);
    ALOGE("%s: enter: flags %#x, is_usb_dev %d, may_use_hifi_record %d,"
    ...
    if (is_usb_dev && (!audio_extn_usb_connected(NULL))) {
        ...
        ALOGE("%s: ignore set device to non existing USB card, use input device(%#x)",
              __func__, devices);
    }
    ...
    *stream_in = NULL;
    ...
    if (in_get_stream(adev, handle) != NULL) {
        ALOGE("%s, input stream already opened", __func__);
        ret = -EEXIST;
    }
    pthread_mutex_unlock(&adev->lock);
    if (ret)
    ...
    ALOGE("%s: enter: sample_rate(%d) channel_mask(%#x) devices(%#x)\
    ...
    ALOGE("%s: source %d, config->channel_mask %#x", __func__, source, config->channel_mask);
	ALOGE("%s:is_usb_dev:%d && may_use_hifi_record:%d\n", __func__, is_usb_dev, may_use_hifi_record);
    ...
    if ((config->sample_rate == LOW_LATENCY_CAPTURE_SAMPLE_RATE) &&
        ...
        ALOGE("%s: USECASE_AUDIO_RECORD_MMAP", __func__);
    } else if (in->realtime) {
        in->config = pcm_config_audio_capture_rt;
        in->config.format = pcm_format_from_audio_format(config->format);
        in->af_period_multiplier = af_period_multiplier;
        ...
        if (!ret_val) {
           if (channel_mask_updated == true) {
               ALOGE("%s: return error to retry with updated channel mask (%#x)",
                   __func__, config->channel_mask);
               ret = -EINVAL;
               goto err_open;
           }
           ALOGE("%s: created multi-channel session succesfully",__func__);
        ...
        if (audio_extn_is_concurrent_capture_enabled()) {
            ...
            if (in->usecase == USECASE_AUDIO_RECORD) {
                ...
                if (!(adev->pcm_record_uc_state)) {
                    ALOGE("%s: using USECASE_AUDIO_RECORD",__func__);
                    ...
                    ALOGE("%s: overriding usecase with USECASE_AUDIO_RECORD_COMPRESS2 and appending compress flag", __func__);
                                    ...
                                    struct audio_stream_in *stream)
{
    ...
    ALOGE("%s: enter:stream_handle(%p)",__func__, in);
    ...
    if (in_ctxt != NULL) {
        ...
        ALOGE("%s, input stream already closed", __func__);
    }
    free(stream);
    pthread_mutex_unlock(&adev->lock);
    return;
            ...
            *pparams = NULL;
            ...
            if (retval >= 0) {
                *pparams = pcm_params_get(card_id, device_id, flags_dir);
                ...
                if (*pparams) {
                    ALOGE("%s: (%s) card %d  device %d", __func__,
                            dir ? "input" : "output", card_id, device_id);
                    pcm_params_to_string(*pparams, info, ARRAY_SIZE(info));
                } else {
                    ALOGE("%s: cannot locate card %d  device %d", __func__, card_id, device_id);
                }
#endif
            }
...
static int check_a2dp_restore_l(struct audio_device *adev, struct stream_out *out, bool restore)
{
    ...
    if (restore) {
        ...
        if ((out->devices & AUDIO_DEVICE_OUT_ALL_A2DP) &&
            ...
            ALOGE("%s: restoring A2dp and unmuting stream", __func__);
        ...
        if (out->flags & AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD) {
            ...
            if (!out->a2dp_compress_mute && !out->standby) {
                ALOGE("%s: selecting speaker and muting stream", __func__);
                devices = out->devices;
                out->devices = AUDIO_DEVICE_OUT_SPEAKER;
                left_p = out->volume_l;
...
{
    pthread_mutex_lock(&adev->lock);
    ALOGE("%s: battery status changed to %scharging", __func__, charging ? "" : "not ");
                     ...
                     hw_device_t **device)
{
    ...
    if (access(VISUALIZER_LIBRARY_PATH, R_OK) == 0) {
        ...
        if (adev->visualizer_lib == NULL) {
            ...
            ALOGE("%s: DLOPEN successful for %s", __func__, VISUALIZER_LIBRARY_PATH);
    ...
    if (access(ADM_LIBRARY_PATH, R_OK) == 0) {
        ...
        if (adev->adm_lib == NULL) {
            ...
            ALOGE("%s: DLOPEN successful for %s", __func__, ADM_LIBRARY_PATH);
    ...
    *device = &adev->device.common;
    ...
    if (property_get("vendor.audio_hal.period_multiplier",value,NULL) > 0) {
        ...
        ALOGE("new period_multiplier = %d", af_period_multiplier);
    }
