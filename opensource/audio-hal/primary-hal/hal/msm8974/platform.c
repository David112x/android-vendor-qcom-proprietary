...
bool platform_send_gain_dep_cal(void *platform, int level) {
    ...
    if (!voice_is_in_call(adev)) {
        ALOGE("%s: Not Voice call usecase, apply new cal for level %d",
               __func__, level);
        ...
        list_for_each(node, &adev->usecase_list) {
            ...
            if (usecase != NULL && usecase->stream.out &&
                ...
                ALOGE("%s: out device is %d", __func__,  usecase->out_snd_device);
...
void platform_set_gsm_mode(void *platform, bool enable)
{
    ...
    if (my_data->gsm_mode_enabled) {
        ...
        ALOGE("%s: disabling gsm mode", __func__);
                                 ...
                                 audio_devices_t out_device)
{
    ...
    if (strcmp(my_data->ec_ref_mixer_path, "")) {
        ALOGE("%s: disabling %s", __func__, my_data->ec_ref_mixer_path);
...
static struct csd_data *open_csd_client(bool i2s_ext_modem)
{
    ...
    if (csd->csd_client == NULL) {
        ...
        ALOGE("%s: DLOPEN successful for %s", __func__, LIB_CSD_CLIENT);
                                      ...
                                      struct platform_data *plat_data)
{
    ...
    ALOGE("%s, is_i2s_ext_modem:%d soundcard name is %s",__func__,
...
static bool is_auto_snd_card(const char *snd_card_name)
{
    ...
    if (!strncmp(snd_card_name, "sa6155-adp-star-snd-card",
        ...
        ALOGE("%s : Auto snd card detected: soundcard name is %s",__func__,
               snd_card_name);
    }
...
static int init_be_dai_name_table(struct audio_device *adev)
{
    ...
    if (be_dai_name_table != NULL) {
        ...
        ALOGE("%s: Successfully got %s, number of be dais is %d\n",
...
void *platform_init(struct audio_device *adev)
{
    ...
    if (my_data->acdb_handle == NULL) {
        ...
        ALOGE("%s: DLOPEN successful for %s", __func__, LIB_ACDB_LOADER);
        ...
        if (!my_data->acdb_send_gain_dep_cal)
            ALOGE("%s: Could not find the symbol acdb_loader_send_gain_dep_cal from %s",
                  __func__, LIB_ACDB_LOADER);
        ...
        if (params &&
            ...
            while (params->info.usecase_id[i] != 0) {
                if (params->info.usecase_id[i] == info->usecase_id[0]) {
                    ALOGE("%s: found params with ip_ch %d op_ch %d uc_id %d snd_dev %d",
        ...
        if (params &&
            ...
            while (params->in_info.usecase_id[i] != 0) {
                if (params->in_info.usecase_id[i] == info->usecase_id[0]) {
                    ALOGE("%s: found params with op_ch %d uc_id %d",
...
bool platform_check_backends_match(snd_device_t snd_device1, snd_device_t snd_device2)
{
    ...
    ALOGE("%s: snd_device1 = %s, snd_device2 = %s", __func__,
    ...
    ALOGE("%s: be_itf1 = %s, be_itf2 = %s, match %d", __func__, be_itf1, be_itf2, result);
    return result;
}
...
int platform_set_fluence_type(void *platform, char *value)
{
    ...
    ALOGE("%s: fluence type:%d", __func__, my_data->fluence_type);
    ...
    if (!strncmp(AUDIO_PARAMETER_VALUE_DUALMIC,value, sizeof(AUDIO_PARAMETER_VALUE_DUALMIC))) {
        if (!strncmp("fluencepro", my_data->fluence_cap, sizeof("fluencepro")) ||
            ...
            ALOGE("fluence dualmic feature enabled \n");
    ...
    if (fluence_type != my_data->fluence_type) {
        ALOGE("%s: Updating fluence_type to :%d", __func__, fluence_type);
                                      ...
                                      effect_type_t effect_type)
{
    ...
    ALOGE("%s: snd_device = %d module_id = %d",
...
int platform_set_snd_device_acdb_id(snd_device_t snd_device, unsigned int acdb_id)
{
    ...
    ALOGE("%s: acdb_device_table[%s]: old = %d new = %d", __func__,
                                      ...
                                      effect_type_t effect_type)
{
    ...
    ALOGE("%s 0x%x 0x%x 0x%x 0x%x", __func__, effect_config.module_id,
...
int platform_get_meta_info_key_from_list(void *platform, char *mod_name)
{
    ...
    ALOGE("%s: for module %s", __func__, mod_name);
...
{
    ALOGE("%s: Platform: %p, type: %d", __func__, platform, type);
    if(type == PCM_CAPTURE)
...
int platform_get_snd_device_acdb_id(snd_device_t snd_device)
{
    ...
    if (operator_specific_device_table[snd_device] != NULL)
    {
		ALOGE("ldx %s line:%d snd_device:%d", __func__, __LINE__, snd_device);
...
int platform_get_native_support()
{
    ...
    ALOGE("%s:napb: ui Prop enabled(%d) version(%d)", __func__,
                             ...
                             char *value, int len)
{
    ...
    if (ret >= 0) {
        if (na_props.platform_na_prop_enabled) {
            ...
            ALOGE("%s:napb: na_props.ui_na_prop_enabled: %d", __func__,
                  na_props.ui_na_prop_enabled);
        } else {
            str_parms_add_str(reply, AUDIO_PARAMETER_KEY_NATIVE_AUDIO,
                              "false");
            ALOGE("%s:napb: native audio not supported: %d", __func__,
...
int platform_get_backend_index(snd_device_t snd_device)
{
    ...
    ALOGE("%s:napb: backend port - %d device - %d ", __func__, port, snd_device);
    return port;
}
                                    ...
                                    int app_type)
{
    ...
    for (i = 0; i < num_devices; i++) {
        ...
        if (usecase->id == USECASE_AUDIO_SPKR_CALIB_TX) {
            ALOGE("Reset backend cfg sample rate to 8KHz for spkr calib Tx use case");
        ...
        ALOGE("%s: sending audio calibration for snd_device(%d) acdb_id(%d)",
...
int platform_set_voice_volume(void *platform, int volume)
{
    ...
    ALOGE("%s: Setting voice volume index: %ld", __func__, set_values[0]);
    ...
    ALOGE("%s: Setting RX Device Mute to: %ld", __func__, set_values[0]);
    mixer_ctl_set_array(ctl, set_values, ARRAY_SIZE(set_values));
...
int platform_set_mic_mute(void *platform, bool state)
{
    ...
    if (!ctl) {
        ...
        ALOGE("%s Setting voice mute state: %d", __func__, state);
        mixer_ctl_set_array(ctl, set_values, ARRAY_SIZE(set_values));
    }
...
int platform_set_device_mute(void *platform, bool state, char *dir)
{
    ...
    ALOGE("%s: Setting device mute state: %d, mixer ctrl:%s",
...
snd_device_t platform_get_output_snd_device(void *platform, struct stream_out *out)
{
    ...
    ALOGE("%s: enter: output devices(%#x)", __func__, devices);
...
exit:
    ALOGE("%s: exit: snd_device(%s)", __func__, device_table[snd_device]);
    return snd_device;
}
                                           ...
                                           audio_devices_t out_device)
{
    ...
    ALOGE("%s: enter: out_device(%#x) in_device(%#x) channel_count (%d) channel_mask (0x%x) mode:%x source:%x",
          __func__, out_device, in_device, channel_count, channel_mask, mode, source);
    ...
    if ((out_device != AUDIO_DEVICE_NONE) && ((mode == AUDIO_MODE_IN_CALL) ||
		...
		ALOGE("ldx %s line:%d in_device:0x%x", __func__, __LINE__, in_device);
        in_device = AUDIO_DEVICE_IN_USB_HEADSET;//((out_device == AUDIO_DEVICE_NONE) ?
        ...
        {    snd_device = get_snd_device_for_voice_comm(my_data, in, out_device, in_device);
			ALOGE("ldx %s line:%d snd_device:0x%x", __func__, __LINE__, snd_device);
...
int platform_set_hdmi_channels(void *platform,  int channel_count)
{
    ...
    ALOGE("Ext disp channel count: %s", channel_cnt_str);
...
int platform_edid_get_max_channels(void *platform)
{
    ...
    if(ret == 0 && info != NULL) {
        for (i = 0; i < info->audio_blocks && i < MAX_EDID_BLOCKS; i++) {
            ALOGE("%s:format %d channel %d", __func__,
...
static int platform_set_slowtalk(struct platform_data *my_data, bool state)
{
    ...
    if (!ctl) {
        ...
        ALOGE("Setting slowtalk state: %d", state);
        ret = mixer_ctl_set_array(ctl, set_values, ARRAY_SIZE(set_values));
...
static int set_hd_voice(struct platform_data *my_data, bool state)
{
    ...
    if (!ctl) {
        ...
        ALOGE("Setting HD Voice state: %d", state);
        ret = mixer_ctl_set_array(ctl, set_values, ARRAY_SIZE(set_values));
...
int platform_set_parameters(void *platform, struct str_parms *parms)
{
    ...
    ALOGE_IF(kv_pairs != NULL, "%s: enter: %s", __func__, kv_pairs);
    ...
    if (err >= 0) {
        ...
        if (my_data->hd_voice != state) {
            ...
            ALOGE("%s: HD Voice already set to %d", __func__, state);
        }
    }
    ...
    if (err >= 0) {
        ...
        ALOGE("%s: recieved update of external audio device %s %s",
                         __func__,
    ...
    if (err >= 0) {
        ...
        ALOGE("%s: add operator[%s] mccmnc[%s]", __func__, info->name, info->mccmnc);
    }
    ...
    if (err >= 0) {
        ...
        ALOGE("%s: max_mic_count %d", __func__, my_data->max_mic_count);
    }
    ...
    ALOGE("%s: exit with code(%d)", __func__, ret);
    if(kv_pairs != NULL)
        free(kv_pairs);
    if(value != NULL)
        free(value);
    ...
    if (!ctl) {
        ...
        for (i = 0; i < num_ctl_values; i++) {
            if (mixer_ctl_set_value(ctl, i, session_id)) {
                ALOGE("Error: invalid session_id: %x", session_id);
                ret = -EINVAL;
                break;
            }
        }
                            ...
                            struct str_parms *reply)
{
    ...
    ALOGE_IF(kv_pairs != NULL, "%s: exit: returns - %s", __func__, kv_pairs);
    free(kv_pairs);
}
...
{
    ALOGE("%s: input source :%d", __func__, source);
...
bool platform_sound_trigger_usecase_needs_event(audio_usecase_t uc_id)
{
    ...
    case USECASE_AUDIO_RECORD_VOIP:
        ...
        break;
    default:
        ALOGE("%s:usecase_id[%d] no need to raise event.", __func__, uc_id);
    }
    return needs_event;
}
...
uint32_t platform_get_compress_offload_buffer_size(audio_offload_info_t* info)
{
    ...
    if(fragment_size < MIN_COMPRESS_OFFLOAD_FRAGMENT_SIZE)
        ...
        fragment_size = MAX_COMPRESS_OFFLOAD_FRAGMENT_SIZE;
    ALOGE("%s: fragment_size %d", __func__, fragment_size);
                         ...
                         snd_device_t snd_device, struct audio_backend_cfg backend_cfg)
{
    ...
    if (adev_device_cfg_ptr->use_client_dev_cfg) {
        ALOGE("%s::: Updating with the config set by client "
    ...
    if (backend_idx == HDMI_RX_BACKEND) {
        ...
        ALOGE("%s: Format doesnt have to be set", __func__);
    }
                         ...
                         struct audio_backend_cfg *backend_cfg)
{
    ...
    ALOGE("%s:becf: afe: bitwidth %d, samplerate %d channels %d format %d"
                                   ...
                                   struct audio_backend_cfg *backend_cfg)
{
    ...
    if (backend_idx == USB_AUDIO_RX_BACKEND) {
        ...
        ALOGE("%s: USB BE configured as bit_width(%d)sample_rate(%d)channels(%d)",
    ...
    adev_device_cfg_ptr += backend_idx;
    if (adev_device_cfg_ptr->use_client_dev_cfg) {
        ALOGE("%s: Codec backend needs to be updated as Client provided "
                                   ...
                                   snd_device_t snd_device)
{
    ...
    if ((voice_is_in_call(adev) || adev->mode == AUDIO_MODE_IN_COMMUNICATION)) {
        ...
        list_for_each(node, &adev->usecase_list) {
            ...
            if (in != NULL && uc->type == PCM_CAPTURE &&
                ...
                ALOGE("%s:txbecf: uc %s, id %d, sr %d, bw %d, ch %d, device %s",
    ...
    if (backend_idx == USB_AUDIO_TX_BACKEND) {
        ...
        ALOGE("%s:txbecf: afe: USB BE configured as bit_width(%d)sample_rate(%d)channels(%d)",
              __func__, bit_width, sample_rate, channels);
    }
                                               ...
                                               int snd_id, uint8_t *input_channel_map)
{
    ...
    if (*input_channel_map) {
        for (i = 0; i < channels; i++) {
             ALOGE("%s:: Channel Map channel_map[%d] - %d", __func__, i, *input_channel_map);
...
int platform_set_channel_map(void *platform, int ch_count, char *ch_map, int snd_id, int be_idx)
{
    ...
    struct platform_data *my_data = (struct platform_data *)platform;
    struct audio_device *adev = my_data->adev;
    ALOGE("%s channel_count:%d",__func__, ch_count);
...
unsigned char platform_map_to_edid_format(int audio_format)
{
    ...
    case AUDIO_FORMAT_AC3:
        ALOGE("%s: AC3", __func__);
        format = AC3;
        break;
    case AUDIO_FORMAT_AAC:
        ALOGE("%s:AAC", __func__);
        format = AAC;
        break;
    case AUDIO_FORMAT_AAC_ADTS:
        ALOGE("%s:AAC_ADTS", __func__);
        format = AAC;
        break;
    case AUDIO_FORMAT_E_AC3:
    case AUDIO_FORMAT_E_AC3_JOC:
        ALOGE("%s:E_AC3", __func__);
        format = DOLBY_DIGITAL_PLUS;
        break;
    case AUDIO_FORMAT_DOLBY_TRUEHD:
        ALOGE("%s:MAT", __func__);
        format = MAT;
        break;
    case AUDIO_FORMAT_DTS:
        ALOGE("%s:DTS", __func__);
        format = DTS;
        break;
    case AUDIO_FORMAT_DTS_HD:
        ALOGE("%s:DTS_HD", __func__);
        format = DTS_HD;
        break;
    case AUDIO_FORMAT_PCM_16_BIT:
    case AUDIO_FORMAT_PCM_24_BIT_PACKED:
    case AUDIO_FORMAT_PCM_8_24_BIT:
        ALOGE("%s:PCM", __func__);
        format = LPCM;
        break;
    case AUDIO_FORMAT_IEC61937:
        ALOGE("%s:IEC61937", __func__);
        format = 0;
        break;
    default:
        format =  -1;
    ...
    if (ret == 0 && info != NULL) {
        for (i = 0; i < info->audio_blocks && i < MAX_EDID_BLOCKS; i++) {
            ...
            if (info->audio_blocks_array[i].format_id == format_id) {
                ALOGE("%s:returns true %x",
                      __func__, format);
                return true;
            }
        }
    }
    ALOGE("%s:returns false %x",
           __func__, format);
    return false;
}
...
int platform_set_edid_channels_configuration(void *platform, int channels, int backend_idx, snd_device_t snd_device) {
    ...
    if(ret == 0 && info != NULL) {
        if ((channels > 2) && (channels <= MAX_HDMI_CHANNEL_CNT)) {
            ALOGE("%s:able to get HDMI/DP sink capabilities multi channel playback",
                   __func__);
            max_supported_channels = platform_edid_get_max_channels(my_data);
            if (channels > max_supported_channels)
                channels = max_supported_channels;
            ...
            ALOGEV("%s:channels:%d", __func__, channels);
            ...
            if (adev_device_cfg_ptr->use_client_dev_cfg) {
                ALOGE("%s:: Setting client selected CA %d", __func__,
...
int platform_set_device_params(struct stream_out *out, int param, int value)
{
    ...
    ALOGE("%s: Setting device pp params param: %d, value %d mixer ctrl:%s",
    ...
    if ((out_snd_device == SND_DEVICE_OUT_USB_HEADSET) ||
        ...
        if (property_get_bool("vendor.audio.usb.disable.sidetone", 0)) {
            ...
            if (ret) {
                ...
                ALOGE("%s: No USB sidetone supported, switching to AFE sidetone",
                        __func__);
        ...
        ALOGE("%s: sidetone out device(%d) mixer cmd = %s\n",
              __func__, out_snd_device, str);

        if (enable) {
            ret = audio_route_apply_and_update_path(adev->audio_route, str);
            if (ret) {
                ALOGE("%s: No device sidetone supported, switching to AFE sidetone",
                        __func__);
                audio_route_apply_and_update_path(adev->audio_route, AFE_SIDETONE_MIXER_PATH);
            }
        }
       ...
       void* data, int length, bool persist)
{
    ...
    if (my_data->acdb_set_audio_cal) {
        ...
        if (ret == 0) {
            ...
            ret = my_data->acdb_set_audio_cal((void*)cal, data, (uint32_t)length);
            if (persist && (ret != 0)) {
                ALOGE("[%s] audio cal stored with success, ignore set cal failure", __func__);
                ret = 0;
            }
        }
    }
...
int platform_set_swap_mixer(struct audio_device *adev, bool swap_channels)
{
    ...
    ALOGE("platfor_force_swap_channel :: Channel orientation ( %s ) ",
...
int platform_set_swap_channels(struct audio_device *adev, bool swap_channels)
{
    ...
    if (my_data) {
        if (!hw_info_is_stereo_spkr(my_data->hw_info)) {
            ALOGE("%s: will not swap due to it is not stereo spkr", __func__);
            return 0;
        }
    } else {
    ...
    if ((adev->mode == AUDIO_MODE_IN_COMMUNICATION) || (adev->mode == AUDIO_MODE_IN_CALL)) {
        ALOGE("%s: will not swap due to audio mode %d", __func__, adev->mode);
        return 0;
    }
...
bool platform_add_gain_level_mapping(struct amp_db_and_gain_table *tbl_entry) {
    ...
    ALOGE("%s: enter .. add %f %f %d", __func__, tbl_entry->amp, tbl_entry->db, tbl_entry->level);
...
int platform_get_gain_level_mapping(struct amp_db_and_gain_table *mapping_tbl,
                                    int table_size) {
    int itt = 0;
    ALOGE("platform_get_gain_level_mapping called ");
    ...
    for (; itt < num_gain_tbl_entry && itt <= table_size; itt++) {
        ...
        ALOGE("%s: added amp[%f] db[%f] level[%d]", __func__,
