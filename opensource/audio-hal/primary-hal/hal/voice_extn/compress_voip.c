...
static int voip_set_mode(struct audio_device *adev, int format)
{
    ...
    ALOGE("%s: enter, format=%d", __func__, format);

    mode = audio_format_to_voip_mode(format);
    ALOGE("%s: Derived mode = %d", __func__, mode);
...
static int voip_set_rate(struct audio_device *adev, int rate)
{
    ...
    ALOGE("%s: enter, rate=%d", __func__, rate);
...
static int voip_set_dtx(struct audio_device *adev, bool enable)
{
    ...
    ALOGE("%s: enter, enable=%d", __func__, enable);
                                                ...
                                                struct str_parms *reply)
{
    ...
    ALOGE_IF(kv_pairs != NULL, "%s: exit: return - %s", __func__, kv_pairs);
    free(kv_pairs);
}
...
bool compress_voip_pcm_prop_check()
{
    ...
    {
        ALOGE("%s: VoIP PCM property is enabled", __func__);
        return true;
    }
    else
        return false;
...
static int voip_stop_call(struct audio_device *adev)
{
    ...
    ALOGE("%s: enter, out_stream_count=%d, in_stream_count=%d",
                           ...
                           struct pcm_config *voip_config)
{
    ...
    if (uc_info == NULL) {
        ...
        if (uc_info->in_snd_device == SND_DEVICE_NONE &&
            uc_info->out_snd_device == SND_DEVICE_NONE) {
            ALOGE("No valid input output device return");
            ret = -EIO;
        ...
        //ALOGE("%s: ldx Invalid PCM devices (rx: %d tx: %d) for the usecase(%d)",
        ...
        ALOGE("%s: Opening PCM capture device card_id(%d) device_id(%d)",
		...
		adev->snd_card = 1;
		pcm_dev_rx_id = 0;
        ALOGE("%s: Opening PCM playback device card_id(%d) device_id(%d)",
